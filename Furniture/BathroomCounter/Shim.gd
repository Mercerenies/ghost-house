...
func _draw() -> void:
    ...
    match counter.get_shim_channel():
        counter.ShimChannel.BathroomCounterNS:
            var above = room.get_entity_cell(pos + Vector2(0, -1))
            var below = room.get_entity_cell(pos + Vector2(0, 2))
            if above is Furniture and above.get_shim_channel() == counter.ShimChannel.BathroomCounterNS:
                draw_texture_rect_region(image,
                                         Rect2(0, 0, 32, 32),
                                         FurnitureShim.index_to_rect(INDEX_TOP))
            if below is Furniture and below.get_shim_channel() == counter.ShimChannel.BathroomCounterNS:
                draw_texture_rect_region(image,
                                         Rect2(0, 32, 32, 32),
                                         FurnitureShim.index_to_rect(INDEX_BOTTOM))
        counter.ShimChannel.BathroomCounterWE:
            var left = room.get_entity_cell(pos + Vector2(-1, 0))
            var right = room.get_entity_cell(pos + Vector2(2, 0))
            if left is Furniture and left.get_shim_channel() == counter.ShimChannel.BathroomCounterWE:
                draw_texture_rect_region(image,
                                         Rect2(0, 0, 32, 32),
                                         FurnitureShim.index_to_rect(INDEX_LEFT_FRONT if frame == 0 else INDEX_LEFT))
            if right is Furniture and right.get_shim_channel() == counter.ShimChannel.BathroomCounterWE:
                draw_texture_rect_region(image,
                                         Rect2(32, 0, 32, 32),
